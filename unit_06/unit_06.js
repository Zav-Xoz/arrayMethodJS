...
const f7 = () => {
    z7 = [];
    a7.filter(item => {
        // console.log(item.pass.length)
        if (item.pass.length <= 6) z7.push(item);
    })
    console.log(z7);
}
...
const f10 = () => {
    z10 = {};
    for (let key in a10) {
        // console.log(`KEY  ${key} ==  Value  ${a10[key]}`);
        if (callback10(a10[key]) > 5) z10[key] = a10[key];
    }
    console.log(z10);
}
